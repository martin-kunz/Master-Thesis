    ...
    for bug_id, test_results in result.items():
        ...
        for filename, test_result in test_results.items():
            ...
            if isinstance(test_result, str):
                result_processed[bug_id][filename].update({
                    'parse_error': True,
                    'compile_error': False,
                    'has_error': True,
                    'buggy_output': None,
            ...
            if isinstance(test_result, dict) and 'buggy' in test_result and isinstance(test_result['buggy'], dict) and 'failed_tests' in test_result['buggy']:
                result_processed[bug_id][filename]['fib_test_id'] = test_result['buggy']['failed_tests'][0] if len(
                    test_result['buggy']['failed_tests']) > 0 else None
            else:
                result_processed[bug_id][filename]['fib_test_id'] = None

            if 'buggy' not in test_result or not isinstance(test_result['buggy'], dict):
                result_processed[bug_id][filename].update({
                    'parse_error': False,
                    'compile_error': True, 
                    'has_error': True,
                    'buggy_output': str(test_result.get('buggy')),
                    'exception_type': None,
                    'value_matching': None,
                    'exception_msg': None,
                    'is_fib': False,
            ...
            fixed_val = test_result.get('fixed')
            if isinstance(fixed_val, dict):
                compile_error_in_fixed = fixed_val.get('compile_error', False)
                runtime_error_in_fixed = fixed_val.get('runtime_error', False)
            else:
                compile_error_in_fixed = False
                runtime_error_in_fixed = False

            error_in_fixed = compile_error_in_fixed or runtime_error_in_fixed

            result_processed[bug_id][filename]['parse_error'] = False
            
            buggy_compile_error = test_result['buggy'].get('compile_error', False)
            buggy_runtime_error = test_result['buggy'].get('runtime_error', False)

            result_processed[bug_id][filename]['compile_error'] = buggy_compile_error or compile_error_in_fixed
            result_processed[bug_id][filename]['has_error'] = buggy_compile_error or buggy_runtime_error or error_in_fixed
            
            result_processed[bug_id][filename]['test_file_path'] = test_file
            result_processed[bug_id][filename]['buggy_output'] = test_result['buggy'].get('fib_error_msg')
            result_processed[bug_id][filename]['exception_type'] = test_result['buggy'].get('exception_type')
            result_processed[bug_id][filename]['value_matching'] = test_result['buggy'].get('value_matching')
            result_processed[bug_id][filename]['exception_msg'] = test_result['buggy'].get('failure_message')

            result_processed[bug_id][filename]['is_fib'] = test_result['buggy'].get('autogen_failed', False) and (not error_in_fixed)
            result_processed[bug_id][filename]['success'] = test_result.get('success', False) and (not error_in_fixed)
