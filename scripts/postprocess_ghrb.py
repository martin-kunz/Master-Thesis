    ...
    if args.all:
        ...
        for bug_key, tests in tqdm(bug2tests.items()):
            ...
            try:
                ...
                parts = project.rsplit('_', 1)
                if len(parts) == 2 and parts[1].isdigit():
                    project = parts[0]
                    bug_id_int = int(parts[1])
                else:
                    import re

                    m = re.search(r'(\d+)', bug_id)
                    if m:
                        bug_id_int = int(m.group(1))
                    else:
                        raise ValueError(f"Cannot extract bug ID: {bug_id}")
            bug_id = bug_id_int
            ...
            bug_key_lookup = f'{project}-{bug_id}'
            if bug_key_lookup not in data:
                continue
            target_bug = data[bug_key_lookup]            
        ...
        bug_key_lookup = f'{args.project}-{args.bug_id}'
        if bug_key_lookup not in data:
            print(f"Error: Bug {bug_key_lookup} nicht in verified_bugs.json gefunden!")
            exit(1)
