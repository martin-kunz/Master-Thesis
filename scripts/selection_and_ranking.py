...
x = ""
run = "repro_xxx"
RESULT_PATH = f'../../results/{run}.json'
GEN_TEST_PATH = f'../data/Defects4J/gen_tests_repro/'
RESULT_PATH = f'../../results/d4j_{run}_02.json'
GEN_TEST_PATH = f'../data/Defects4J/gen_tests_d4j_{run}/'

RESULT_PATH_GHRB = '/vol/fob-vol7/mi21/kunzmart/Master-Thesis/libro/results/{run}_GHRB.json'
GEN_TEST_PATH_GHRB = '../data/GHRB/gen_tests_{run}_bereinigt/'
...
def collect_ranking_features(fib_bug_ids, fib_clusters, aggreement_scores, OB, parsed_output):
    ...
    for bug_id in fib_bug_ids:
        ...
        for rep, test_paths in fib_clusters[bug_id].items():
            ...
            if rep_test not in aggreement_scores[bug_id]:
                continue 
...
def sort_unique_fib_tests(bug_id, test_features, test_clusters):
    ...
    for c, test_paths in test_clusters[bug_id].items():
        ...
        if not test_keys: # Check if the list is empty
            continue
        test_feat = test_features.loc[test_keys[0]]
...
if __name__ == "__main__":
    ...
    for bug_id in fib_bug_ids:
        ...
        for name, test_result in result_dict[bug_id].items():
            if test_result['buggy_output'] is not None:
                try:
                    if dname == 'd4j':
                        parsed_output[bug_id][name] = parse_buggy_output(test_result['buggy_output'], mode='d4j')
                    elif dname == 'ghrb':
                        parsed_output[bug_id][name] = parse_buggy_output(test_result['buggy_output'], 
                            exception_type=test_result['exception_type'], 
                            exception_msg=test_result['exception_msg'], 
                            value_matching=test_result['value_matching'], 
                            mode='ghrb')
                except (IndexError, ValueError, AttributeError):
                    parsed_output[bug_id][name] = {
                        'is_crash': False,
    ...
    with open(f'../../results/{run}_ranking_features_{dname}.csv', 'w') as f:
        rank_feature_df.to_csv(f, index=False)

    if rank_feature_df.empty:
        rank_df = pd.DataFrame(columns=['bug_id', 'test_name', 'rank', 'first_success_rank', 'total_success_fibs', 'num_clusters'])
    else:
        rank_df = rank_tests_using_clusters(rank_feature_df, test_clusters)

    with open(f'../../results/{run}_ranking_{dname}.csv', 'w') as f:
    ...
    if rank_feature_df.empty:
        rank_df_selected = pd.DataFrame(columns=['bug_id', 'test_name', 'rank', 'first_success_rank', 'total_success_fibs', 'num_clusters'])
    else:
        rank_feature_df_selected = select_confident_bugs(rank_feature_df, threshold=SELECTION_THRESHOLD)
        test_clusters_selected = {b_id: test_clusters[b_id] for b_id in rank_feature_df_selected.bug_id.unique()}
        rank_df_selected = rank_tests_using_clusters(rank_feature_df_selected, test_clusters_selected)

    with open(f'../../results/{run}_ranking_{dname}_selected_th{SELECTION_THRESHOLD}.csv', 'w') as f:
    ...
    for N in Ns:
        ...
        if total > 0:
            ...
            print(f'* acc@{N}: {acc_N} (0.0)')

    success_count = len(rank_df_selected[rank_df_selected.first_success_rank < BIG_NUMBER])

    if total > 0:
        print(f'* Total (success): {success_count} ({round(success_count / total, 2)})')
    else:
        print(f'* Total (success): {success_count} (0.0)')
